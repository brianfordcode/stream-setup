...
window.addEventListener('mousedown', function (event) {
  let dragElement = event.composedPath().find(element => element._vei && element._vei.on__vuedrag)

  if (!dragElement) return

  parentSVG = dragElement.closest('svg')

  dragListener = dragElement ? dragElement._vei.on__vuedrag : null
  dropListener = dragElement ? dragElement._vei.on__vuedrop : null
